...
export const actions = {
    ...
    fetchAllTypes({ commit }) {
        return this.$api.getAllTypeTraitement().then((data) => {
    ...
    searchTypes({ commit }, { page, s }) {
        ...
        return this.$api.searchAllTypeTraitementPage(page, s).then((data) => {

            commit("SET_SEARCHED_TYPES", data);
        });
    },
    fetchTypes({ commit }, page) {
        return this.$api.selectAllTypeTraitementPage(page).then((data) => {
    ...
    fetchType({ commit, getters }, id) {
        ...
        if (operation) {
            ...
            return this.$api.getTypeTraitement(id).then((data) => {
